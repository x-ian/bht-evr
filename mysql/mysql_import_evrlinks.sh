...
cp /tmp/village_neighbours/evr_links_raw_2 /tmp/evr_links_raw.csv
mysqlimport  -u root -proot  --fields-terminated-by=',' --local $MYSQL_DB /tmp/evr_links_raw.csv

mysql -u $MYSQL_USER -p$MYSQL_PW $MYSQL_DB -e "update evr_links_raw set name = substring(name,1, instr(name,'_')-1) where name like '%\\_%';"
